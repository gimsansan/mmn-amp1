...
| 높이 | `CHART_HEIGHT = 132` |
| 점 | 반지름 약 3.5. **마지막 점만** 조금 더 큼 |
| 선 | `theme.accent`, 두께 2.5, round join/cap |
| 마지막 점 채움 | `theme.highlight` |
| 그 외 점 채움 | `theme.surface`, 테두리 `theme.accent` |
...
6. 선 아래 `LinearGradient` 면. `useId`로 그라데이션 id가 안 겹치게 함.
