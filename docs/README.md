...
| **누적형** (그때의 기록) | **위에 새로 추가**한다(최신이 위). **과거 블록은 절대 수정하지 않는다** | impl-log, 수정 리뷰 |
...
| **④ 기록** | 무엇을 했나 / 어떻게 고쳤나 | `impl-log_1.md`(이어서), `impl-log.md`(과거), `fix-reviews.md`, `공개저장소-제거목록.md` |
...
| `analysis/` (7개 + README) | 갱신 | 7축을 이 앱 주요 파일에 **적용한 결과**(화면·데이터·상태·에러·타입·성능·테스트). 지침 원본 `analaysys/`는 2026-08-24 삭제 | **이 앱이 각 축에서 실제로 어떻게 생겼나** |
...
| `공개저장소-제거목록.md` | 누적 | 2026-08-24 공개 정리로 뺀 내부 문서 목록 | **무엇을 왜 뺐나** |
...
| 새 채팅으로 넘길 맥락 | **`impl-log_1.md`** 맨 위 항목 | 인계 파일(`handoff*.md`)은 2026-08-24 폐지. 다시 만들지 않음 |
...
4. **인계 파일 폐지**(2026-08-24) — `handoff*.md` 삭제. 다시 만들지 않음. 세션 기록은 `impl-log_1.md`.
...
1. [`impl-log_1.md`](./impl-log_1.md) **맨 위 항목**
...
| **누적형** 문서(impl-log·fix-reviews) | **그때의 기록**. 지금과 다를 수 있고 그래도 고치지 않는다. 「지금 어떤가」는 백로그에서 볼 것 |
...
> - `handoff-YYYY-MM-DD.md` 날짜 사본을 만들던 규칙 → **폐지**(2026-08-07). `handoff*.md` 자체는 2026-08-24 공개 정리로 삭제.
