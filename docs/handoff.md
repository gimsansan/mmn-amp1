...
> **2026-08-19부터 새 인계는 `docs/handoff3.md`.** (`handoff2.md`는 2026-08-18~19 과거.) 이 파일에는 새 블록을 넣지 않는다.
> **방식(과거)**: 「인계문 작성해」 시 **덮어쓰지 말고 최신 블록을 맨 위에 추가**. 각 블록 제목: `## 인계 — YYYY-MM-DD HH:mm`.  
...
> **읽을 때**: 여기 블록은 **그때의 기록**이다. 지금 코드·방침과 다를 수 있고, 그래도 고치지 않는다(누적형). 현재 상태는 `improvement-backlog.md`, 최신 인계는 `handoff4.md`.

---

## 인계 — 2026-08-14 13:35

새 채팅 AI용. **이번 세션 = 종료 확인창 + 건별 삭제 + 통계 카피·하단 버튼 윤곽. 트랙 필터 등은 안 함.** JS만, 리빌드 불필요.

### 합의

- 집계=측정만 → 제목 「측정 통계」·라벨 「측정 횟수」(사용자 직접).
- 「기록 전체 삭제」=저장 키 통째 → 목록·그래프·집계 전부. 연습만 지우는 게 아님.
- 끝내기·중지 오탭 → 확인창 후에만 종료. 로직(`onEndManual`) 그대로.
- 건별 삭제: 연습·측정 가리지 않음. 측정 건 삭제 시 그 값만 빠지고 앞뒤 점이 **바로 이어짐**(날짜 빈칸 아님, x는 균등 간격). 같은 트랙 그래프만 변동.
- 트랙 필터·전체삭제 분리·P2-5·다시 듣기 UI **안 함**(사용자).
- 통계 하단 3버튼: 흰 면 유지. 새로고침·연습 목록=잉크 글자+윤곽. 전체 삭제=danger `#9B3B3B` 글자+윤곽. 홈 통계 아이콘(삼색 막대)은 손대지 않음.
- 커밋은 사용자 직접.

### 한 일

- `confirmEndSession` → 세 트랙 끝내기·중지.
- `deleteSavedSession(id)` + 카드 아래 「삭제」(확인 Alert). 테스트 포함(`sessionStore` 38).
- 백로그 P2-7: 건별 삭제 반영, 트랙 필터는 남음.
- `theme.danger` · `ActionButton` `variant="danger"` · `outlineMatchLabel`(통계 하단 3버튼만).
- 커밋됨: `592620c` feat(history): 통계를 측정 기준으로 맞추고 건별 삭제·종료 확인을 넣음.

### 안 한 일 / 미커밋

- 트랙 필터·전체삭제 분리·P2-5·다시 듣기 — 안 하기로 함.
- **미커밋**: 훈련 3종 아이콘 분리(`bars`/`findTone`/`vibrate`, 선택→듣기 준비→시작 동일 그림) · 하단 버튼 danger/윤곽 · `theme.danger`.

### 핵심 경로

- `src/training/confirmEndSession.ts` · `sessionStore.ts` · `SessionHistoryScreen.tsx` · `action-button.tsx` · `icon.tsx` · `src/app/index.tsx`

### 다음

- 미커밋 아이콘·버튼 윤곽 커밋은 사용자.
- 실기기: 종료 확인 취소/확인, 건별 삭제 후 그래프, 하단 잉크/빨강 윤곽.

### 단정 금지

- `미검증`: 실기기 확인창·삭제 후 그래프·버튼 대비.
- **리빌드 불필요.**
