...
|------|------|
| 트랙 | 공통·인프라 |
| 근거·결정 | 사용자: 연습 탭의 높낮이 비교·다른 음 찾기를 소리 높낮이 탭으로. 화면 재사용(`PitchCompareScreen`·`FreqSessionScreen`·듣기 준비). 저장·통계는 `sessionStore` 그대로. 통계 진입은 연습 탭 유지. 탭이 둘 다 중첩 화면이므로 하드웨어 뒤로가기는 `useFocusEffect`로 포커스된 탭만. |
| 변경 요약 | PTA 탭에 음고 2종 선택 목록. 연습 탭은 떨림 찾기 + 통계. |
| 주요 경로 | `src/training/pta/PtaSessionScreen.tsx`, `src/app/index.tsx`, `src/components/app-tabs.tsx`, `src/app/_layout.tsx` |
| 결과 | 코드 이동 완료. 실기기 확인은 이 세션에서 안 봄. |
| 확인 | 린트(이동 후 useEffect 제거). |
| 단정 금지 | `추정`: JS만이라 **리빌드 불필요**. `미검증`: 실기기에서 탭 전환·하드웨어 뒤로·듣기 준비·측정 저장→통계. `주의`: NativeTabs가 탭을 살려 두면 연습 탭에 남은 세션 상태는 다시 들어갔을 때 그대로일 수 있음(기존과 같음). |
| 성능·주의 | 없음(기존 화면 재사용, 정적 목록). **dev client 리빌드 불필요.** |
| 다음 | 실기기에서 소리 높낮이 탭 2카드·연습 탭 떨림만. 통계에 음고 측정이 붙는지. |

### 2026-08-18 14:59 — README 대상 기기 문구

| 항목 | 내용 |
|------|------|
| 트랙 | 문서 |
| 근거·결정 | 사용자: README 「저사양 안드로이드」→「중간 사용 안드로이드」. README §경량화 방침만 수정. |
| 변경 요약 | 대상 기기 표현을 중간 사용 안드로이드로 바꿈. |
| 주요 경로 | `README.md` |
| 결과 | README 한 줄 반영. |
| 확인 | 해당 문장만. |
| 단정 금지 | `주의`: `docs/dev-client-setup-context.md`·`.cursor/rules` 등 다른 파일은 아직 「저사양」. 요청 범위는 README만. |
| 성능·주의 | 없음. **리빌드 불필요.** |
| 다음 | 다른 문서도 맞출지. |

### 2026-08-18 14:52 — 단어인지도 하단 탭 라우트

| 항목 | 내용 |
|------|------|
| 트랙 | 공통·인프라 |
| 근거·결정 | 사용자: 「단어인지도」탭 생성. PTA와 같이 expo-router 파일 라우트 + `NativeTabs.Trigger name="wrs"`. 세션·자극은 자리 화면만. 탭 순서 링 6 · PTA · 단어인지도 · 연습. |
| 변경 요약 | `wrs.tsx`가 `WrsSessionScreen`을 가리키게 하고, 하단 4탭에 단어인지도를 넣음. |
| 주요 경로 | `src/app/wrs.tsx`, `src/components/app-tabs.tsx`, `src/training/wrs/WrsSessionScreen.tsx`, `src/app/_layout.tsx`, `src/app/index.tsx` |
| 결과 | 코드 연결 완료. 실기기 탭 표시는 이 세션에서 안 봄. |
| 확인 | 파일·import 경로만. |
| 단정 금지 | `추정`: NativeTabs 트리거 추가는 JS 라우팅이라 **리빌드 불필요**. `미검증`: 실기기에서 4탭·긴 라벨「단어인지도」·`md="chat"` 아이콘. `주의`: 자리 화면뿐. WRS 훈련·저장 없음. |
| 성능·주의 | 없음(정적 자리 화면). 네이티브·의존성·리소스 변경 없음 → **dev client 리빌드 불필요**(JS만, 핫리로드). |
| 다음 | 단어인지도 세션 화면·자극. 탭 라벨이 좁은 기기에서 잘리는지. |

### 2026-08-18 14:26 — PTA 하단 탭 라우트

| 항목 | 내용 |
|------|------|
| 트랙 | 공통·인프라 |
| 근거·결정 | 사용자: `src/app/pta.tsx`·`src/training/pta/` 생성 후 라우팅 요청. 링 6와 같이 expo-router 파일 라우트 + `NativeTabs.Trigger name="pta"`. 폴더가 비어 import가 깨지므로 자리 화면만 둠(세션·자극 없음). 탭 순서 링 6 · PTA · 연습. |
| 변경 요약 | `pta.tsx`가 `PtaSessionScreen`을 가리키게 하고, 하단 3탭에 PTA를 넣음. |
| 주요 경로 | `src/app/pta.tsx`, `src/components/app-tabs.tsx`, `src/training/pta/PtaSessionScreen.tsx`, `src/app/_layout.tsx`, `src/app/index.tsx` |
| 결과 | 코드 연결 완료. 실기기 탭 표시는 이 세션에서 안 봄. |
| 확인 | 파일·import 경로만. |
| 단정 금지 | `추정`: NativeTabs 트리거 추가는 JS 라우팅이라 **리빌드 불필요**. `미검증`: 실기기에서 3탭·`md="hearing"` 아이콘. `주의`: PTA는 자리 화면뿐. 역치·진단형으로 읽히지 않게 훈련 카피는 아직 없음. |
| 성능·주의 | 없음(정적 자리 화면). 네이티브·의존성·리소스 변경 없음 → **dev client 리빌드 불필요**(JS만, 핫리로드). |
| 다음 | PTA 세션 화면·자극. 탭 라벨·아이콘·순서 확정. |

### 2026-08-18 14:09 — 인계 정본을 handoff2.md로 고정

| 항목 | 내용 |
|------|------|
| 트랙 | 문서·규칙 |
| 근거·결정 | 사용자: 링 6 인계를 `handoff.md`에 넣지 말 것. `handoff2.md`에 기록. 「인계문 작성해」를 안 해도 최신 `handoff2`에 저장. |
| 변경 요약 | `handoff.md`의 14:01 블록 삭제 후 `handoff2.md` 상단으로 이전. 규칙 파일 저장 경로를 `handoff2.md`로 바꿈. |
| 주요 경로 | `docs/handoff2.md` · `docs/handoff.md` · `.cursor/rules/android-dev-client.mdc` |
| 결과 | 파일 반영. |
| 확인 | 14:01이 `handoff.md`에 없는지·`handoff2.md` 상단에 있는지. |
| 단정 금지 | `주의`: `handoff.md`는 과거 로그로 남김. 사용자가 다시 열라고 하기 전 추가 금지. |
| 성능·주의 | 없음. **리빌드 불필요.** |
| 다음 | 이후 인계는 `handoff2.md`만. |

### 2026-08-18 13:50 — 링 6 진행 패널을 이틀 미리보기로 채움

| 항목 | 내용 |
|------|------|
| 트랙 | 링 6 |
| 근거·결정 | 사용자: 추이·그리드가 **이틀** 보이게. 모자란 날 결과는 랜덤이어도 됨. 저장소는 건드리지 않고 화면만 채움. |
| 변경 요약 | 실기록이 0~1일이면 전날을 시드 난수로 채워 그리드·추이선을 그림. 2일 이상이면 실기록만. idle은 기록 없어도 패널을 띄움. |
| 주요 경로 | `src/training/ling6/Ling6ProgressPanel.tsx` · `Ling6SessionScreen.tsx` |
| 결과 | 코드 반영. 실기기 확인은 안 함. |
| 확인 | 린트. |
| 단정 금지 | `주의`: 미리보기 칸은 AsyncStorage에 안 남음. `주의`: 실기록 1일에 붙는 전날은 가짜라 통계로 읽히면 안 됨. |
| 성능·주의 | 없음. **리빌드 불필요**(JS). |
| 다음 | 실기기에서 가로축 이틀·추이선 확인. |
