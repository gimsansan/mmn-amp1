/**
 * 통계 읽기 창구 — 저장소 4개(`ling6Store` · `sessionStore` · `wrsStore` ·
 * `twoCharStore`)를 **그대로 두고** 읽는 쪽만 한 곳으로 모은다.
...
export type StatsGroup = "ling6" | "pitch" | "wrs" | "am";

/** 칩 순서 = 하단 탭 순서(링 6 · 소리 높낮이 · 단어 듣기 · 떨림). */
...
/**
 * 한 번 읽어 둔 네 저장소. 칩을 바꿔도 다시 읽지 않는다.
...
/**
 * 네 저장소를 한 번에 읽는다. 레코드가 전부 요약 숫자라 4키 합쳐도 수십 KB다.
 * 하나가 깨져도 나머지는 보여 준다(그 종목만 빈 목록).
 */
export async function loadStatsFeed(): Promise<StatsFeed> {
  const [ling6, sessions, wrs1, wrs2] = await Promise.all([
